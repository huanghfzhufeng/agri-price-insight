...
export default function Sidebar({ navItems, user, onLogout, onClose }) {
  return (
    <aside className="flex h-full w-72 flex-col rounded-r-[32px] bg-[var(--md-surface-container)] p-6 shadow-sm">
      ...
      <div className="mt-auto border-t border-[var(--md-surface-container-low)] pt-6">
        <div className="flex items-center gap-3 rounded-2xl px-2 py-2 transition-colors hover:bg-[var(--md-surface-container-low)]">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--md-primary)] text-sm font-medium text-white">
            管
          </div>
          <div>
            <p className="text-sm font-medium text-[#1C1B1F]">{user?.display_name || "系统管理员"}</p>
        ...
        <button
          type="button"
          onClick={onLogout}
          className="mt-4 flex w-full items-center gap-3 rounded-full px-4 py-3 text-sm font-medium text-[#49454F] transition-colors hover:bg-[var(--md-surface-container-low)] hover:text-[#1C1B1F]"
        >
          <LogOut size={18} />
          退出登录
        </button>
